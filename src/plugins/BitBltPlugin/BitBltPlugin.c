/* Automatically generated by
	SmartSyntaxPluginCodeGenerator VMMaker.oscogLLP64-nice.1932 uuid: 642ecc37-5300-3643-8d95-5acb4704c924
   from
	BitBltSimulation VMMaker.oscogLLP64-nice.1932 uuid: 642ecc37-5300-3643-8d95-5acb4704c924
 */
static char __buildInfo[] = "BitBltSimulation VMMaker.oscogLLP64-nice.1932 uuid: 642ecc37-5300-3643-8d95-5acb4704c924 " __DATE__ ;
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"BitBltPlugin VMMaker.oscogLLP64-nice.1932 (i)"
#else
	"BitBltPlugin VMMaker.oscogLLP64-nice.1932 (e)"
#endif
;
	...
	for (i = 1; i <= bbH; i += 1) {
		...
		do {
			...
			if (mapperFlags == (ColorMapPresent | ColorMapIndexedPart)) {
				...
				do {
					sourcePix = (sourceWord >> srcShift1) & sourcePixMask;
...
pixSwapwith(unsigned int sourceWord, unsigned int destWord)
{
	...
	/* mask low pixel */
	lowMask = (1ULL << destDepth) - 1;

	/* mask high pixel */
	highMask = ((usqInt)(lowMask) << ((destPPW - 1) * destDepth));
	shift = 32 - destDepth;
	result = result | ((((usqInt)((destWord & lowMask)) << shift)) | ((destWord & highMask) >> shift));
	if (destPPW <= 2) {
		return result;
	}
...
rgbAddwith(unsigned int sourceWord, unsigned int destinationWord)
{
	...
	if (destDepth < 16) {
		...
		componentMask = (1ULL << destDepth) - 1;
...
rgbComponentAlphawith(sqInt sourceWord, sqInt destinationWord)
{
	...
	result = 0;
	for (i = 1; i <= destPPW; i += 1) {
		p1 = ((((unsigned int) sourceWord)) & mask) >> ((i - 1) * destDepth);
		p2 = ((((unsigned int) destinationWord)) & mask) >> ((i - 1) * destDepth);
		if (!(destDepth == 32)) {
			if (destDepth == 16) {
				p1 = (((((usqInt)((p1 & 0x1F)) << 3)) | (((usqInt)((p1 & 0x3E0)) << 6))) | (((usqInt)((p1 & 0x7C00)) << 9))) | 0xFF000000U;
				p2 = (((((usqInt)((p2 & 0x1F)) << 3)) | (((usqInt)((p2 & 0x3E0)) << 6))) | (((usqInt)((p2 & 0x7C00)) << 9))) | 0xFF000000U;
...
rgbMaxwith(unsigned int sourceWord, unsigned int destinationWord)
{
	...
	if (destDepth == 16) {
		...
		return (partitionedMaxwithnBitsnPartitions(sourceWord, destinationWord, 5, 3)) + (((usqInt)((partitionedMaxwithnBitsnPartitions(sourceWord >> 16, destinationWord >> 16, 5, 3))) << 16));
	}
	else {
...
rgbMinInvertwith(unsigned int wordToInvert, unsigned int destinationWord)
{
	...
	if (destDepth == 16) {
		...
		return (partitionedMinwithnBitsnPartitions(sourceWord, destinationWord, 5, 3)) + (((usqInt)((partitionedMinwithnBitsnPartitions(sourceWord >> 16, destinationWord >> 16, 5, 3))) << 16));
	}
	else {
...
rgbMulwith(unsigned int sourceWord, unsigned int destinationWord)
{
	...
	if (destDepth == 16) {
		...
		return (partitionedMulwithnBitsnPartitions(sourceWord, destinationWord, 5, 3)) + (((usqInt)((partitionedMulwithnBitsnPartitions(sourceWord >> 16, destinationWord >> 16, 5, 3))) << 16));
	}
	else {
...
rgbSubwith(unsigned int sourceWord, unsigned int destinationWord)
{
	...
	if (destDepth == 16) {
		...
		return (partitionedSubfromnBitsnPartitions(sourceWord, destinationWord, 5, 3)) + (((usqInt)((partitionedSubfromnBitsnPartitions(sourceWord >> 16, destinationWord >> 16, 5, 3))) << 16));
	}
	else {
...
l14:	/* end deltaFrom:to:nSteps: */;
	...
	if ((methodArgumentCount()) == 2) {
		...
		if (sourceMapOop == (nilObject())) {
			...
			if ((slotSizeOf(sourceMapOop)) < (1ULL << sourceDepth)) {
	...
	warpAlignMask = (1ULL << warpAlignShift) - 1;
			...
			if (smoothingCount == 1) {
				...
				if (mapperFlags == (ColorMapPresent | ColorMapIndexedPart)) {
					...
					do {
						...
						sourceWord = long32At(srcIndex);
						srcBitShift = warpBitShiftTable[x & warpAlignMask];
						sourcePix1 = (sourceWord >> srcBitShift) & warpSrcMask;
					...
					do {
						...
						sourcePix2 = (sourceWord1 >> srcBitShift) & warpSrcMask;
