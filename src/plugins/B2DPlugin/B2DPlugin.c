/* Automatically generated by
   ...
   from
	BalloonEnginePlugin VMMaker.oscogLLP64-nice.1932 uuid: 642ecc37-5300-3643-8d95-5acb4704c924
 */
static char __buildInfo[] = "BalloonEnginePlugin VMMaker.oscogLLP64-nice.1932 uuid: 642ecc37-5300-3643-8d95-5acb4704c924 " __DATE__ ;
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"B2DPlugin VMMaker.oscogLLP64-nice.1932 (i)"
#else
	"B2DPlugin VMMaker.oscogLLP64-nice.1932 (e)"
#endif
;
...
adjustAALevel(void)
{
	...
	x0 = ((unsigned int)(workBuffer[GWSpanStart])) >> (workBuffer[GWAAShift]);
	x1 = ((unsigned int)(workBuffer[GWSpanEnd])) >> (workBuffer[GWAAShift]);
	while (x0 < x1) {
		pixelValue = spanBuffer[x0];
		spanBuffer[x0] = (pixelValue | ((pixelValue >> adjustShift) & adjustMask));
		x0 += 1;
	}
	return 0;
}
...
bitmapValuebitsatXy(sqInt bmFill, sqInt bits, sqInt xp, sqInt yp)
{
	...
	cMask = (1ULL << bmDepth) - 1;
	rShift = (32 - bmDepth) - ((xp & ((1ULL << rShift) - 1)) * bmDepth);
...
clearSpanBuffer(void)
{
	...
	x0 = ((unsigned int)(workBuffer[GWSpanStart])) >> (workBuffer[GWAAShift]);
	x1 = (((unsigned int)(workBuffer[GWSpanEnd])) >> (workBuffer[GWAAShift])) + 1;
	if (x0 < 0) {
		x0 = 0;
	}
...
displaySpanBufferAt(sqInt y)
{
	...
	targetX0 = ((unsigned int)(workBuffer[GWSpanStart])) >> (workBuffer[GWAAShift]);
	if (targetX0 < (workBuffer[GWClipMinX])) {
		targetX0 = workBuffer[GWClipMinX];
	}
	targetX1 = ((unsigned int)(((workBuffer[GWSpanEnd]) + (workBuffer[GWAALevel])) - 1)) >> (workBuffer[GWAAShift]);
...
drawWideEdgefrom(sqInt edge, sqInt leftX)
{
	...
	/* Not for the moment */
	type = ((unsigned int)((objBuffer[edge + GEObjectType]) & GEPrimitiveTypeMask)) >> 1;
...
{
	return ((unsigned int)((objBuffer[edge + GEObjectType]) & GEPrimitiveTypeMask)) >> 1;
}
...
l16:	/* end aaFirstPixelFrom:to: */;
	...
	cMask = (((unsigned int)(workBuffer[GWAAColorMask])) >> (workBuffer[GWAAShift])) | 4042322160U;
	...
	while (x < lastPixel) {
		...
		if ((xp >= 0)
		 ...
		 && (yp < bmHeight)))) {
			...
			/* rShift - shift value to move the pixel in the word to the lowest bit position */
			cMask2 = (1ULL << bmDepth1) - 1;
			rShift1 = (32 - bmDepth1) - ((xp & ((1ULL << rShift1) - 1)) * bmDepth1);
	...
	while (x < rightX) {
		...
		if ((xp >= 0)
		 ...
		 && (yp < bmHeight)))) {
			...
			/* rShift - shift value to move the pixel in the word to the lowest bit position */
			cMask3 = (1ULL << bmDepth2) - 1;
			rShift2 = (32 - bmDepth2) - ((xp & ((1ULL << rShift2) - 1)) * bmDepth2);
...
l1:	/* end aaFirstPixelFrom:to: */;
	...
	if (x < lastPixel) {
		colorMask = (((unsigned int)(workBuffer[GWAAColorMask])) >> (workBuffer[GWAAShift])) | 4042322160U;
...
fillSpanfromto(unsigned int fill, sqInt leftX, sqInt rightX)
{
	...
	l1:	/* end fillColorSpan:from:to: */;
		...
		type = ((unsigned int)(((objBuffer[fill + GEObjectType]) & GEPrimitiveTypeMask) & GEPrimitiveFillMask)) >> 8;
		if (type <= 1) {
			return 1;
		}
		switch (type) {
...
{
	return ((unsigned int)(((objBuffer[fill + GEObjectType]) & GEPrimitiveTypeMask) & GEPrimitiveFillMask)) >> 8;
}
