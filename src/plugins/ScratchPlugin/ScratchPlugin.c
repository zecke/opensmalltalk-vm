/* Automatically generated by
   ...
   from
	ScratchPlugin VMMaker.oscogLLP64-nice.1919 uuid: 67128742-82db-1c4a-a5e7-f1f7a5cee80c
 */
static char __buildInfo[] = "ScratchPlugin VMMaker.oscogLLP64-nice.1919 uuid: 67128742-82db-1c4a-a5e7-f1f7a5cee80c " __DATE__ ;
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"ScratchPlugin VMMaker.oscogLLP64-nice.1919 (i)"
#else
	"ScratchPlugin VMMaker.oscogLLP64-nice.1919 (e)"
#endif
;
	...
	for (outY = 0; outY < outH; outY += 1) {
		...
		for (outX = 0; outX < outW; outX += 1) {
			...
			if (tWeight < 500000) {
				...
				t = (((w1 * ((((unsigned int)p1) >> 16) & 0xFF)) + (w2 * ((((unsigned int)p2) >> 16) & 0xFF))) + (w3 * ((((unsigned int)p3) >> 16) & 0xFF))) + (w4 * ((((unsigned int)p4) >> 16) & 0xFF));
				outPix = ((sqInt)((usqInt)(((((usqInt)t) >> 20) & 0xFF)) << 16));
				t = (((w1 * ((((unsigned int)p1) >> 8) & 0xFF)) + (w2 * ((((unsigned int)p2) >> 8) & 0xFF))) + (w3 * ((((unsigned int)p3) >> 8) & 0xFF))) + (w4 * ((((unsigned int)p4) >> 8) & 0xFF));
